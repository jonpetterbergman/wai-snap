...
import Snap.Core(MonadSnap,method,modifyResponse,getsRequest,getHeader,Method,modifyRequest,addHeader)
...
defaultOptions = CORSOptions
  ...
  , corsAllowedMethods =
      return $ HashSet.fromList $ map (HashableMethod . Right)
...
applyCORS options m =
  (join . fmap decodeOrigin <$> (getsRequest $ getHeader "Origin")) >>= maybe m corsRequestFrom
 ...
 where
  ...
  corsRequestFrom origin = do
    ...
    if origin `inOriginList` originList
       then method (Right OPTIONS) (preflightRequestFrom origin)
              <|> handleRequestFrom origin
       else m

  preflightRequestFrom origin = do
    maybeMethod <- fmap (parseMethod . Char8.unpack) <$>
                     (getsRequest $ getHeader "Access-Control-Request-Method")
          ...
          then do
            maybeHeaders <-
              fromMaybe (Just HashSet.empty) . fmap splitHeaders
                <$> (getsRequest $ getHeader "Access-Control-Request-Headers")
  ...
  addAccessControlAllowOrigin origin = modifyRequest $ 
    addHeader "Access-Control-Allow-Origin"
              (encodeUtf8 $ Text.pack $ show origin)

  addAccessControlAllowCredentials = do
    allowCredentials <- corsAllowCredentials options
    when (allowCredentials) $ modifyRequest $ addHeader "Access-Control-Allow-Credentials" "true"
  ...
  commaSepHeader k f vs =
    case vs of
      [] -> return ()
      _  -> modifyRequest $ addHeader k $ Char8.intercalate ", " (map f vs)
...
parseMethod "GET"     = HashableMethod $ Right GET
parseMethod "POST"    = HashableMethod $ Right POST
parseMethod "HEAD"    = HashableMethod $ Right HEAD
parseMethod "PUT"     = HashableMethod $ Right PUT
parseMethod "DELETE"  = HashableMethod $ Right DELETE
parseMethod "TRACE"   = HashableMethod $ Right TRACE
parseMethod "OPTIONS" = HashableMethod $ Right OPTIONS
parseMethod "CONNECT" = HashableMethod $ Right CONNECT
parseMethod "PATCH"   = HashableMethod $ Right PATCH
parseMethod s         = HashableMethod $ Left $ Char8.pack s
...
instance Hashable HashableMethod where
  hashWithSalt s (HashableMethod (Right GET))        = s `hashWithSalt` (0 :: Int)
  hashWithSalt s (HashableMethod (Right HEAD))       = s `hashWithSalt` (1 :: Int)
  hashWithSalt s (HashableMethod (Right POST))       = s `hashWithSalt` (2 :: Int)
  hashWithSalt s (HashableMethod (Right PUT))        = s `hashWithSalt` (3 :: Int)
  hashWithSalt s (HashableMethod (Right DELETE))     = s `hashWithSalt` (4 :: Int)
  hashWithSalt s (HashableMethod (Right TRACE))      = s `hashWithSalt` (5 :: Int)
  hashWithSalt s (HashableMethod (Right OPTIONS))    = s `hashWithSalt` (6 :: Int)
  hashWithSalt s (HashableMethod (Right CONNECT))    = s `hashWithSalt` (7 :: Int)
  hashWithSalt s (HashableMethod (Right PATCH))      = s `hashWithSalt` (8 :: Int)
  hashWithSalt s (HashableMethod (Left m)) =
